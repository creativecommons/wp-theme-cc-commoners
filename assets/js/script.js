jQuery(document).ready(function ($) {
      ...
      success: function (data) {
        var target = $("#" + targetId),
          link =
            '<a href="' +
            data +
            '" target="_blank" class="voucher_profile_link">View User profile</a>';
        console.log(target);
        target.html("");
        target.html(link);
      },
  ...
  $("#set-new-vouch-reason").on("click", function (e) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data == "error") {
          $("#change-voucher-messages")
            .append(
              '<div class="error notice is-dismissible"><p>There was an error sending your request</p></div>'
            )
            .find(".notice")
            .delay(3200)
            .fadeOut(300);
  ...
  $("#input_41_1").on("change", function (e) {
    ...
    if (!$("#" + profileUrlId).length) {
      $('<span id="' + profileUrlId + '"></span>').insertAfter(
        "#" + thisSelectId + "_chosen"
      );
    }
    $(otherSelectId + ' option[disabled="disabled"]').removeAttr("disabled");
    $(otherSelectId + ' option[value="' + selectedValue + '"]').attr(
      "disabled",
      "disabled"
    );

	...
	<div class="container">
		<div class="columns">
			<div class="column is-12">
				<div class="entry-content content text-format">
					<?php the_content(); ?>
							...
							if ( ! empty( $log_user ) ) {
								...
								foreach ( $log_user as $log_item ) {
									...
									if ( ! $is_in_check_array ) {
										if ( ( $check_status['applicant_id'] == $log_item['applicant_id'] ) && ( $check_status['status'] == 0 ) ) {
											continue; }
										$check_ids[] = $log_item['applicant_id'];
										echo '<div class="column is-one-third">';
											echo '<article class="hentry entry-ask-request card entry-post">';
										...
										if ( isset( $_GET['clarification'] ) && ( esc_attr( $_GET['applicant_id'] ) == $log_item['applicant_id'] ) ) {
											...
											if ( $user_clarification_mode ) {
														...
														echo '<p class="control"><textarea name="clarification_voucher" id="clarification_voucher" cols="50" rows="10" class="textarea">' . $entries['entry_text'] . '</textarea></p>';
											...
											$change_url = add_query_arg(
												array(
													'clarification' => true,
													'applicant_id' => $log_item['applicant_id'],
												),
